...
public class SearchPatActivity extends AppCompatActivity {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        searchView.setQueryHint(Html.fromHtml("<font color = #000000>" + "Cauta..." + "</font>"));
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.option_Dentiste:
                DoctorAdapterFiltred.specialiteSearch = true;
                adapter.getFilter().filter("Stomatolog");
                return true;
            case R.id.option_Ophtalmologue:
                DoctorAdapterFiltred.specialiteSearch = true;
                adapter.getFilter().filter("Oftalmolog");
                return true;
            ...
            case R.id.option_Pédiatre:
                DoctorAdapterFiltred.specialiteSearch = true;
                adapter.getFilter().filter("Pediatru");
                return true;
            case R.id.option_Radiologue:
                DoctorAdapterFiltred.specialiteSearch = true;
                adapter.getFilter().filter("Radiolog");
                return true;
            case R.id.option_Rhumatologue:
                DoctorAdapterFiltred.specialiteSearch = true;
                adapter.getFilter().filter("Reumatolog");
                return true;
            default:
    ...
    private void configureToolbar(){
        ...
        toolbar.setTitle("Medici disponibili");
        // Sets the Toolbar
